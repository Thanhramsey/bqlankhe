...
{
    private array $headers = ['Số thứ tự', 'Mã hộ *', 'Họ tên *', 'Số điện thoại', 'Địa chỉ *', 'Địa chỉ HĐ', 'CCCD', 'Mã tuyến', 'Mã dịch vụ *', 'Email', 'Mã số thuế', 'Người đại diện', 'Ghi chú', 'Trạng thái (HOAT_DONG/NGUNG)'];
    ...
    public function template(): string
    {
        ...
        $sheet->fromArray([[1, 'HD-0001', 'Nguyễn Văn A', '0905000000', '01 Nguyễn Tất Thành, An Khê', 'Trụ sở Công ty A, Gia Lai', '048000000001', 'AK-01', 'RAC-HO', 'vana@example.com', '0400000001', 'Nguyễn Văn A', 'Dữ liệu mẫu', 'HOAT_DONG']], null, 'A2');
        $sheet->freezePane('A2');
        $sheet->setAutoFilter('A1:N2');
        ...
        $widths = [12, 16, 24, 18, 36, 36, 20, 16, 34, 28, 20, 24, 30, 30];
        foreach ($widths as $index => $width) {
            $sheet->getColumnDimension(chr(65 + $index))->setWidth($width);
        }
        $sheet->getStyle('A2:N200')->getAlignment()->setVertical(Alignment::VERTICAL_TOP);
        $sheet->getStyle('A2:N200')->getAlignment()->setWrapText(true);
        ...
        DB::transaction(function () use ($rows, &$errors, &$imported) {
            foreach (array_slice($rows, 1, null, true) as $index => $values) {
                ...
                try {
                    ...
                    $serviceCode = trim((string) ($values[8] ?? ''));
                    ...
                    $household->fill([
                        ...
                        'collection_route_id' => $route?->id, 'email' => $this->nullable($values[9] ?? null), 'tax_code' => $this->nullable($values[10] ?? null),
                        'representative' => $this->nullable($values[11] ?? null), 'note' => $this->nullable($values[12] ?? null),
                        'ward' => 'An Khê', 'is_active' => strtoupper(trim((string) ($values[13] ?? 'HOAT_DONG'))) !== 'NGUNG', 'deleted_at' => null,
                    ]);
