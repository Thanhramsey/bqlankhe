...
export const useAuthStore = defineStore('auth', {
 ...
 actions: {
  async login(email:string,password:string){this.loading=true;try{const r=await api<{token:string;user:Profile}>('/auth/login',{method:'POST',body:JSON.stringify({email,password})});localStorage.setItem('token',r.data.token);this.user=r.data.user}catch(e){localStorage.removeItem('token');throw e}finally{this.loading=false}},
