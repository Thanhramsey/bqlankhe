...
<template><section class="dashboard-view">
  <div class="d-flex flex-column flex-sm-row align-sm-center justify-space-between ga-3 mb-5"><div><h1 class="text-h5 text-md-h4 font-weight-bold mb-1">Dashboard</h1><p class="text-body-2 text-medium-emphasis mb-0">Theo dõi tình hình thu phí, công nợ và hóa đơn</p><div v-if="data?.updated_at" class="text-caption text-medium-emphasis mt-1"><v-icon icon="mdi-clock-outline" size="15"/> Cập nhật {{ new Date(data.updated_at).toLocaleString('vi-VN') }}</div></div><v-btn color="primary" variant="tonal" prepend-icon="mdi-refresh" :loading="loading" @click="load">Làm mới dữ liệu</v-btn></div>
